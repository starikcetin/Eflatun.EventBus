...
namespace Eflatun.EventBus.Dev.Samples.ListenerConfigTest
{
    public class L5S : MonoBehaviour
    {
        ...
        {
            _listenerConfig = EventBus.ListenerConfig.SingleChannelNoBroadcast(0, ListenPhase.Regular);
            _eventBus = eventBus;
        }
        ...
        {
            Debug.Log($"{nameof(L5S)} received {nameof(EventA)} from {sender.GetType().Name}");
        }
    }
}
