...
namespace Eflatun.EventBus.Dev.Samples.ChannelsSample
{
    public class L02 : MonoBehaviour
    {
        ...
        {
            _listenerConfig = ListenerConfig.MultipleChannelsNoBroadcast(new HashSet<int>(new []{ 0, 2 }), ListenPhase.Regular);
            _eventBus = eventBus;
        }
        ...
        {
            Debug.Log($"{metadata.Sender.GetType().Name}\t->\t{nameof(L02)}");
        }
    }
}
