...
namespace Eflatun.EventBus.Dev.Sample.Channels
{
    public class L4 : MonoBehaviour
    {
        ...
        {
            _listenerConfig = ListenerConfig.SpecificChannelsAndBroadcast(new[] {0}, ListenPhase.Regular);
            _eventBus = eventBus;
        }
        ...
        {
            Debug.Log($"{nameof(L4)} received {nameof(EventA)} from {sender.GetType().Name}");
        }
    }
}
